...
public class FictitiousPlayer extends Player {
    ...
    public int getMove(int move) {
        ...
        if(rowPlayer){
            double score_a = prob_a * board.getScore(0,0).getRowScore() + prob_a * board.getScore(0,1).getRowScore();
            double score_b = prob_b * board.getScore(1,0).getRowScore() + prob_b * board.getScore(1,1).getRowScore();
            if (score_a > score_b) return 0;
            return 1;
        }
        else{
            double score_a = prob_a * board.getScore(0,0).getColScore() + prob_a * board.getScore(1,0).getColScore();
